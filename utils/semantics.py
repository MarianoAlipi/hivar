...
def match_operators(left_op, right_op, operand):
    cube = get_cube()
    try:
        res = cube[left_op][right_op][operand]
        if res is ERR:
            raise Exception("operand types did not match")
        return res
    except Exception as err:
        raise Exception(
            'Problem while checking types, the operands probably do not exist. '
            f'left_op: {left_op}, right_op: {right_op}, operand: {operand}. Error: {err}')
